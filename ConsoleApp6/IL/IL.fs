...
let notReserved (str: string) = if Array.contains <| str.ToUpper() <| reservedWords then "_" + str else str

type Label = 
    Local of string | Global of string
    override t.ToString() =
        match t with 
        | Local s -> sprintf "@@%s" s
        | Global s -> notReserved s
...
type Register = 
    ...
    override t.ToString () =
        ...
        | DI -> "di"
        | SI -> "si"
        | Var (name,_) -> sprintf "[%s]" <| notReserved name
module Register =
    let size = function
